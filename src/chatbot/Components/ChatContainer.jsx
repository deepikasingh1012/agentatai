...
const handleMainMenu = () => {
  const firstBotMsg = messages.find(msg => msg.sender === "bot" && msg.options?.length > 0);
  if (firstBotMsg) {
    setMessages([
    ...
    setQuestionHistory([]);
    setSelectedOptions({});
    setCurrentQuestion(firstBotMsg);
  }
};
...
const handlePreviousMenu = () => {
  ...
  // Prepare options (with 'Other' button)
  const botMessage = {
    text: lastQuestion.text,
    sender: 'bot',
    type: lastQuestion.type,
    url: lastQuestion.url,
    question_label: lastQuestion.question_label,
    options: lastQuestion.options && lastQuestion.options.length > 0
      ? [...lastQuestion.options.filter(o => o.id !== "other"), { id: "other", question_text: "Other" }]
...
//     const messageElements = document.querySelectorAll('.chat-box .mb-1');
//     const match = Array.from(messageElements).find(el =>
//       el.textContent.includes(lastQuestion.text)
//     );
//     if (match) {
...
//   setQuestionHistory(updatedHistory);
//   setCurrentQuestion(lastQuestion);
//   scrollToMessage(); // 👈 just scroll instead of printing again
// };
...
// 🔁 Save current question before moving forward
    if (currentMsg) {
        setQuestionHistory(prev => [...prev, currentMsg]);
    }
...
// ✅ Save selected option with children for Previous Menu replay
if (currentMsg) {
  setQuestionHistory(prev => [...prev, currentMsg]);
}
...
{index > 1 && (
      <div className="d-flex justify-content-center gap-2 my-2">
        <button className="btn btn-outline-primary btn-sm" onClick={handleMainMenu}>
          Main Menu
        </button>
        <button className="btn btn-outline-secondary btn-sm" onClick={handlePreviousMenu}>
          Previous Menu
        </button>
      </div>
    )}
